/**
 * PASSE I v7 - Siretisation via Firecrawl Search (Google)
 * Corrections v3:
...
function computeScoreI(candidates, validations, gbRec) {
  ...
  for (const cand of candidates) {
    ...
    // v7: Address mismatch penalty - if SIRENE address exists and differs from GB address
    const gbAddrNorm = normalizeAdresse(gbRec.gb_adresse || gbRec.adresse_gb || '').split(',')[0];
    const sirAddrNorm = val ? normalizeAdresse((val.numero_voie||'') + ' ' + (val.type_voie||'') + ' ' + (val.libelle_voie||'')) : '';
    if (sirAddrNorm && gbAddrNorm && sirAddrNorm.length > 3 && gbAddrNorm.length > 3) {
      if (!gbAddrNorm.includes(sirAddrNorm.split(' ').pop()) && !sirAddrNorm.includes(gbAddrNorm.split(' ').pop())) {
        score -= 10; // Address street name mismatch penalty
      }
    }
    ...
    // Adresse exacte bonus
  if (val.adresseExacte) score += 15;
  // Rank bonus
  ...
  return {
    ...
    methode: 'passe_I_firecrawl_v7', denomination: bestMatch.validation.denomination,
...
async function main() {
  ...
  const outputFile = process.argv[3] || '/home/netit972/siretisation_v18_firecrawl.json';
  const maxRecords = parseInt(process.argv[4]) || 999;

  console.log('=== PASSE I v7 - FIRECRAWL SEARCH ===');
