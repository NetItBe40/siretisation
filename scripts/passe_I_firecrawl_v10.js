/**
 * PASSE I v9 - Siretisation via Firecrawl Search (Google)
 * Corrections v3:
...
function computeScoreI(candidates, validations, gbRec) {
  ...
  // v10: CP-PRIORITY RE-RANKING (improved - no score downgrade)
  ...
  if (!bestMatch.validation.cpMatch) {
    ...
    if (cpCandidate && (cpCandidate.score >= bestMatch.score || bestMatch.score < 80)) {
      console.log(`  v10 CP-PRIORITY: switching from ${bestMatch.siren} (score:${bestMatch.score}, cpMatch:false) to ${cpCandidate.siren} (score:${cpCandidate.score}, cpMatch:true)`);
...
async function main() {
  ...
  console.log('=== PASSE I v9 - FIRECRAWL SEARCH ===');
  ...
  for (let i = 0; i < limited.length; i++) {
    ...
    const cat = rec.gb_categorie || rec.categorie || '';
      ...
      // v9: Extract CP from address text - prefer over gb_code_postal when different
      const adresseCpQuery = extractCpFromAdresse(adresse);
      const effectiveCp = adresseCpQuery || cp;
      if (adresseCpQuery && adresseCpQuery !== cp) {
        console.log(`  v9 CP-FIX: gb_code_postal=${cp} -> using adresse CP=${adresseCpQuery}`);
      }

    // --- Requete 1: nom + adresse ---
      const query1 = `siren siret ${nom} ${adresse.split(',')[0]} ${effectiveCp} ${ville} ${cat}`.substring(0, 200);
    ...
    try {
      ...
      if (bestQ1 < 2 && adresse) {
        ...
        const query2 = `siren siret ${adressePart} ${effectiveCp} ${ville}`.substring(0, 200);
