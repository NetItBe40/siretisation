/**
 * PASSE I v2 - Siretisation via Firecrawl Search (Google)
 * Corrections:
 *  - Score base 55 (au lieu de 40)
...
function computeScoreI(candidates, validations, gbRec) {
  ...
  return {
    ...
    methode: 'passe_I_firecrawl_v2', denomination: bestMatch.validation.denomination,
...
async function main() {
  ...
  const outputFile = process.argv[3] || '/home/netit972/siretisation_v14_firecrawl.json';
  ...
  for (let i = 0; i < limited.length; i++) {
    ...
    const query = `siren siret ${nom} ${adresse.split(',')[0]} ${cp} ${ville} ${cat}`.substring(0, 200);

    try {
      const fcResults = await firecrawlSearch(query);
      const candidates = extractSirens(fcResults);
      const scored = scoreFiche(candidates);
